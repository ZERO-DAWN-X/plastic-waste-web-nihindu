      ...
      <Container>
        <div className="flex h-18 items-center justify-between py-3">
          ...
          <nav className="hidden items-center space-x-1 md:flex">
            ...
            {/* Hide rewards for collectors, show Dashboard for business users */}
            {session?.user?.userType?.toUpperCase() !== 'COLLECTOR' && (
              <NavLink href={session?.user?.userType === 'BUSINESS' ? "/dashboard" : "/rewards"} 
                      icon={session?.user?.userType === 'BUSINESS' ? <LayoutDashboard className="h-4 w-4" /> : <Award className="h-4 w-4" />}>
                {session?.user?.userType === 'BUSINESS' ? "Dashboard" : "Rewards"}
              </NavLink>
            )}
          ...
          >
            <Container>
              <div className="flex flex-col py-4 space-y-4">
                ...
                {/* Hide rewards for collectors, show Dashboard for business users in mobile menu */}
                {session?.user?.userType?.toUpperCase() !== 'COLLECTOR' && (
                  <MobileNavLink href={session?.user?.userType === 'BUSINESS' ? "/dashboard" : "/rewards"} onClick={closeMenu}>
